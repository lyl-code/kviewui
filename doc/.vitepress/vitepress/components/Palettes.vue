...
<template>
  <div class="grid-cols-3 lt-sm:grid-cols-1 grid gap-5 mt-10">
    <div
      v-for="(item, index) in colorPresetNames"
      :key="index"
      class="flex flex-col justify-center items-center"
    >
      <div
        class="text-base rounded-t cursor-pointer hover:scale-105 hover:rounded hover:transition hover:duration-200 text-white mb-0 h-20 px-3 py-2 flex flex-col justify-between dark:text-black w-full"
        :style="{
      ...
      <div
        v-for="(childItem, childIndex) in colors[darkMode ? 'dark' : 'light'][
          item.enName
        ]"
        :key="childIndex"
